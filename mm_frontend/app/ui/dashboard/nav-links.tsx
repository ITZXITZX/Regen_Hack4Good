...
export default function NavLinks() {
  ...
  // Dynamically set the Dashboard href based on user type
  const dashboardHref = isAdmin ? '/dashboard/admin' : '/dashboard/resident';

  const links = [
    { name: 'Dashboard', href: dashboardHref, icon: HomeIcon, int: 22 },
    !isAdmin ? { name: 'Earn Vouchers', href: '/dashboard/resident', icon: BanknotesIcon, int: 15 } : undefined,
    !isAdmin ? { name: 'Buy Products', href: '/dashboard/resident', icon: ShoppingCartIcon, int: 15 } : undefined,
    //isAdmin ? { name: 'Approve Tasks', href: '/dashboard/admin/approvetasks', icon: CheckCircleIcon, int: 15 } : undefined,
    isAdmin ? { name: 'Create Tasks', href: '/dashboard/admin/createtasks', icon: PencilIcon, int: 15 } : undefined,
    isAdmin ? { name: 'Generate Report', href: '/dashboard/admin/generatereport', icon: DocumentArrowDownIcon, int: 2 } : undefined,
  ];
